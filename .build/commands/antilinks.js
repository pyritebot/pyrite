...
export default class AntiLinks {
  constructor() {
    this.data = new SlashCommandBuilder().setName("antilinks").setDescription("Toggle Anti Links in your server (this will block every self promotion link)!").setDefaultMemberPermissions(PermissionFlagsBits.Administrator).addSubcommand((subcommand) => subcommand.setName("on").setDescription("turn anti links on!")).addSubcommand((subcommand) => subcommand.setName("off").setDescription("turn anti links off!"));
  }
