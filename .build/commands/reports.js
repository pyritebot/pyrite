...
export default class Report {
  constructor() {
    this.data = new SlashCommandBuilder().setName("reports").setDescription("Report to spread awareness of potentially toxic users and scammers").addSubcommand(
      (subcommand) => subcommand.setName("add").setDescription("Add a report for a user").addUserOption((option) => option.setName("user").setDescription("The user you want to report").setRequired(true)).addStringOption((option) => option.setName("reason").setDescription("Why do you want to report this user?").setRequired(true)).addAttachmentOption((option) => option.setName("image").setDescription("To approve this report, we need image proof.").setRequired(true))
    ).addSubcommand(
      (subcommand) => subcommand.setName("remove").setDescription("Remove a report for a user").addStringOption((option) => option.setName("id").setDescription("The id of the report you want taken down.").setRequired(true)).addStringOption((option) => option.setName("reason").setDescription("Why do you want to remove this report?").setRequired(true)).addAttachmentOption((option) => option.setName("image").setDescription("To approve this report, we need image proof.").setRequired(true))
    ).addSubcommand(
      (subcommand) => subcommand.setName("show").setDescription("Show all reports of a user!").addUserOption((option) => option.setName("user").setDescription("The mention or user id of the user you want to see reports from"))
    );
  }
    ...
    switch (interaction.options.getSubcommand()) {
      ...
      case "remove":
        ...
        try {
          ...
          const reports = user == null ? void 0 : user.reports;
 ...
 **Reason:** ${reason}`,
      ...
      case "show":
        ...
        if (member == null ? void 0 : member.user.bot) {
        ...
        try {
          const user = await prisma.user.findUnique({
            where: { user: member == null ? void 0 : member.user.id },
            select: { reports: true }
          });
          const reports = user == null ? void 0 : user.reports;
          const show = new EmbedBuilder({
            author: {
              name: (_a = interaction.guild) == null ? void 0 : _a.name,
              icon_url: (_b = interaction.guild) == null ? void 0 : _b.iconURL()
            },
            title: "<:report:1009191992040894657> Reports",
            description: ((reports == null ? void 0 : reports.length) ?? 0) === 0 ? `> ${member == null ? void 0 : member.user} doesn't have any reports!` : `> ${(reports == null ? void 0 : reports.length) === 1 ? `This is the report` : `These are the ${(reports == null ? void 0 : reports.length) ?? 0} reports`} that ${member.user} has!`,
            fields: reports == null ? void 0 : reports.map((report) => ({
              name: report.id,
