...
client.once("ready", async () => {
  var _a2;
  setActivity(client);
  loxt.ready(`on ${(_a2 = client.user) == null ? void 0 : _a2.tag}`);
...
client.on("messageCreate", async (message) => {
  ...
  if (!message.inGuild())
    return;
  if (message.author.id === ((_a2 = client.user) == null ? void 0 : _a2.id))
    return;
  ...
  try {
    ...
    const logs = message.guild.channels.cache.get(guild == null ? void 0 : guild.logs);
...
client.on("messageCreate", async (message) => {
  ...
  try {
    ...
    if (msgCount === limit) {
      await ((_b = message.member) == null ? void 0 : _b.timeout(6e4 * (guild == null ? void 0 : guild.spamMinutes), "spamming"));
    ...
    message.guild.roles.cache.filter((r) => r.permissions.has(PermissionFlagsBits.Administrator)).forEach(
      async (r) => {
        var _a3;
        return await ((_a3 = message.member) == null ? void 0 : _a3.roles.remove(r, "Creating too many channels").catch(async () => await r.setPermissions([], "Creating too many channels").catch(() => {
        })));
      }
    );
    setTimeout(async () => {
      var _a3;
      await ((_a3 = message.member) == null ? void 0 : _a3.timeout(6e4 * (guild == null ? void 0 : guild.spamMinutes), "spamming"));
      const messages = [...message.channel.messages.cache.filter((m) => m.author.id === message.author.id).values()].slice(0, limit);
      await message.channel.bulkDelete(messages);
    }, 2e3);
  }
...
client.on("interactionCreate", async (interaction) => {
  ...
  try {
    ...
    (_b = interaction.guild) == null ? void 0 : _b.channels.cache.forEach((ch) => {
      var _a3;
      const c = ch;
      (_a3 = interaction.guild) == null ? void 0 : _a3.roles.cache.filter((role) => role.id !== "@everyone").forEach((role) => c.permissionOverwrites.edit(role.id, { SendMessages: false }));
    });
    const lockDownEmbed = new EmbedBuilder({
      author: {
        name: (_c = interaction.guild) == null ? void 0 : _c.name,
        icon_url: (_d = interaction.guild) == null ? void 0 : _d.iconURL()
      },
...
client.on("interactionCreate", async (interaction) => {
  if (!interaction.isButton())
    return;
  if (interaction.customId !== "lockdown_cancel")
    return;
...
client.on("interactionCreate", async (interaction) => {
  ...
  switch (interaction.values[0]) {
    case "start":
      helpEmbed.setTitle("<:pyrite:1014918476982394891> Pyrite Bot").setDescription(
    ...
    case "moderation":
      helpEmbed.setTitle("<:moderator:1008717826552504321> Moderation").setDescription("> Here's a list of the moderation commands").setFields(
        ...
        },
        {
          ...
          value: "<:blank:1008721958210383902> <:arrow:1009057573590290452> Warn a server member that was spamming or not following the rules. (alias to `/warns add <user> <reason>`)"
        },
        {
        ...
        },
        {
          ...
          value: "<:blank:1008721958210383902> <:arrow:1009057573590290452> Kick a certain user that got muted to many times or is just trying to raid your server."
        },
        {
      ...
      break;
    case "verification":
      helpEmbed.setTitle("<:check:1008718056891101194> Verification").setDescription("> Here's a list of the verification commands").setFields(
    ...
    case "antiraid":
      helpEmbed.setTitle("<:ban:1020333545887113246> Anti Raid").setDescription("> Here's a list of the anti raid commands").setFields(
        {
          name: "`/joingate on`",
          value: "<:blank:1008721958210383902> <:arrow:1009057573590290452> Turn on raid mode. **Pyrite** will start to kick every new member that joins so only use this if theres a big raid. \n"
        },
        ...
        {
          name: "`/lockdown on`",
          value: "<:blank:1008721958210383902> <:arrow:1009057573590290452> __Only use this if theres a big raid.__ **Pyrite** will fully lockdown the entire server and make sure theres not a raid. \n"
        },
    ...
    case "automod":
      helpEmbed.setTitle("<:muted:1010127791070658570> Automod").setDescription("> Here's a list of the automod commands").setFields(
        ...
        { name: "`/antispam off`", value: "<:blank:1008721958210383902> <:arrow:1009057573590290452> Turn on off anti spam." },
        {
          ...
          value: "<:blank:1008721958210383902> <:arrow:1009057573590290452> Set a limit for the amount of messages a user can send in a couple seconds before they get muted. \n"
        },
        {
          name: "`/antispam minutes <minutes>`",
          value: "<:blank:1008721958210383902> <:arrow:1009057573590290452> Set a number of how many minutes a spammer will be muted for when they spam. \n"
        },
...
try {
  ...
  await rest.put(Routes.applicationCommands((_a = client.user) == null ? void 0 : _a.id), { body: commands.map(({ data }) => data.toJSON()) });
} catch (err) {
  loxt.error(err);
}
