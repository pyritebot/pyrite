...
export default class AntiSpam {
	...
	async purge(message: Message, minutes: number, limit: number) {
	...
	async run(message: Message) {
		...
		if (msgCount === limit) {
						...
						.forEach(async r => {
							await message.member?.roles
								...
								.catch(async () => {
									...
									await r
										.setPermissions([], 'Spamming')
										.catch(() => {})
									await this.purge(message, minutes, limit).catch(() => {})
									AntiSpam.users.set(message.author.id ?? message.webhookId, msgCount);
								})
			...
			const logs = message.guild.channels.cache.get(guild?.logs)
			await logs?.send(
				logBuilder({
