...
export default class Info {
	data = new SlashCommandBuilder()
		...
		.setDescriptionLocalizations({ 'es-ES': 'Información sobre un servidor o un usuario' })
		.addSubcommand(subcommand =>
			subcommand
				.setName('user')
				.setNameLocalizations({ 'es-ES': 'usuario' })
				.setDescription('Get information about a user')
				.setDescriptionLocalizations({ 'es-ES': 'Encuentra información sobre un usuario' })
		...
		.addSubcommand(subcommand =>
			subcommand
				...
				.setDescriptionLocalizations({ 'es-ES': 'Encuentra información sobre un servidor' })
		)
	...
	async run(interaction: ChatInputCommandInteraction) {
		...
		switch (interaction.options.getSubcommand()) {
			case 'user':
				...
				if (!member) {
					await interaction.reply({ embeds: [errorEmbedBuilder("The member you specified doesn't seem to exist, if this problem persists, please contact support")], components: [buttons] });
					return;
				}
						...
						},
		        {
		          ...
		          value: `${emojis.reply1}<t:${Math.floor(member.user.createdTimestamp / 1000) + 3600}:R>`,
						},
		        {
							name: '__Joined Server__',
		          value: `${emojis.reply1}<t:${Math.floor(member.joinedTimestamp / 1000)}:R>`,
						},
		        {
							name: '__Permissions__',
		          value: `${emojis.reply1}${
								Object.keys(PermissionFlagsBits).map(perm =>
                  member.permissions.has(perm) ? `\`${perm}\`` : ''
	              ).filter(p => p !== '').join(', ')
							}`,
						},
		        {
		      ...
		      .setFooter({ 
				...
				await interaction.reply({ embeds: [embed1], components: [
			...
			case 'server':
						...
						},
		        {
		          ...
		          value: `${emojis.reply1}<t:${Math.floor(interaction.guild.createdTimestamp / 1000)}:R>`,
						},
		        {
							name: '__Server ID__',
						...
						},
            {
		          ...
		          value: `${emojis.reply1}${[...interaction.guild.roles.cache.values()].filter(r => r.name !== '@everyone').join(', ')}`,
						},
		        {
							name: '__Server Description__',
		          value: `${emojis.reply1}${interaction.guild.description ?? '(No description provided)'}`,
						},
