...
import { loadImage, defaultError, errorEmbedBuilder } from "../utils.js";
...
export default class Info {
	data = new SlashCommandBuilder()
		...
		.addUserOption(option =>
			option.setName('user').setDescription("You can pass a mention or an id of a user.")
		)
			...
			const username = Buffer.from(`
				<svg width="618" height="437">
					...
					<text x="410" y="140" class="name">
						<tspan class="tag">Danger:</tspan> ${(toxicity?.endsWith('00') ? toxicity?.slice(0, -3) : toxicity?.endsWith('0') ? toxicity?.slice(0, -1) : toxicity) ?? 0}%
		 			</text>
