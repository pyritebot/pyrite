...
export default class Moderate {
	data = new SlashCommandBuilder()
		...
		.setDescriptionLocalizations({
			"es-ES":
				"Cambia el nombre de usuario a uno al que se le pueda hacer ping!",
		})
	...
	async run(interaction: ChatInputCommandInteraction) {
		...
		if (
			...
			await interaction.editReply({
				embeds: [errorEmbedBuilder("You can't moderate an Admin's username")],
			});
			return;
		}
		...
		try {
			await member.setNickname(
				`Moderated Nickname ${cuid().slice(0, 6)}`,
				"Username was not pingable!",
			);
		} catch {
			await interaction.editReply({
				embeds: [errorEmbedBuilder("Can't moderate this user")],
			});
			return;
		}
