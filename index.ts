import { SlashCommandBuilder, ChatInputCommandInteraction, TextChannel, ActivityType } from 'discord.js';
import { Client, GatewayIntentBits, REST, Collection, Routes, EmbedBuilder, Colors, PermissionFlagsBits, AttachmentBuilder } from 'discord.js';
...
client.on('messageCreate', async message => {
	...
	try {
		const oldMsgCount = usersCollection.get(message.author.id) ?? 0;
		if (message.author.bot) return;
	...
	try {
		...
		interaction.guild?.channels.cache.forEach(channel => {
			interaction.guild?.roles.cache
				...
				.forEach(role => channel.permissionOverwrites.edit(role.id, { SendMessages: false }));
		});
